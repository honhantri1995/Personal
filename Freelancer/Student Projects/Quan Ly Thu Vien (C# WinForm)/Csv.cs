...
namespace QuanLyThuVien
{
    ...
    class Csv
    {
        ...
        // và lưu tất cả vào tham số đầu ra là list 'records'
        public bool ReadAll_WithContition(List<List<string>> records, int recordId, string recordValue)
        {
            try
            {
                // Mở file CSV ở chế độ đọc
                using (var sr = new StreamReader(_filePath))
                {
                    ...
                    while (sr.Peek() >= 0)
                    {
                        ...
                        if (items.Count > 0 && items[recordId].CompareTo(recordValue) == 0)
                        {
                            records.Add(items);
                        }
                    }
        ...
        public CsvResult UpdateRecord(string id, List<string> newRecord)
        {
            ...
            try
            {
                ...
                {
                    // Tạo file CSV tạm
                    FileInfo fi = new FileInfo(HangSo.CSV_TMP_PATH);
                    ...
                    using (var sw = new StreamWriter(HangSo.CSV_TMP_PATH, true))
                    {
                        // Trong từng line của file CSV, đọc và lưu thông tin
                        while (sr.Peek() >= 0)
                        {
                ...
                if (!result)
                {
                    File.Delete(HangSo.CSV_TMP_PATH);
                ...
                File.Move(HangSo.CSV_TMP_PATH, _filePath);
